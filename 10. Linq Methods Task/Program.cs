...
namespace empty
{
    internal class Program
    {
        ...
        public class Program1
        {
            ...
            {
                Console.WriteLine("Task In Class");
                //x) rhyta.com ve ya dayrep.com domenlerinde emaili olan borclulari cixartmag
                ...
                Console.WriteLine("1....");
                var m = debtors.Where(d => DateTime.Now.Year - d.BirthDay.Year > 26 && DateTime.Now.Year - d.BirthDay.Year < 36);
                ...
                var ff = debtors.Where(f => f.Debt < 5000);
                ...
                //    3) Butov adi 18 simvoldan cox olan ve telefon nomresinde 2 ve ya 2 - den cox 7 reqemi olan borclulari cixartmaq => !!!/m.Bu serte uygun istifadeci yoxdu ona gorede 2 ayri task yazacam ki fullname 18 simvol ve telefon nomresine
               
                Console.WriteLine("3....");
                var ff123 = debtors.Where(f => f.FullName.Length > 18);
                var ttt = debtors.Where(d => d.Phone.Count(c => c == '7') >= 2).ToList();
                Console.WriteLine("Nomresinde 2 veya 2 den cox '7' Reqemi olanlar !");//Ayri Ayri Yazma sebebi : task3 e uygun user yoxdur! ona gorede sertleri ayri ayri yazmisham!
                foreach (var item1 in ttt) {
                    Console.WriteLine(item1);
                }
                Console.WriteLine("18 Simvoldan Cox Olan FullNames");//
                foreach (var item1 in ff123)
                {
                    Console.WriteLine(item1);
                }
                Console.WriteLine();
                ...
                Console.WriteLine("4....");
                var check = debtors.Where(ha => ha.BirthDay.Month == 12 || ha.BirthDay.Month == 01 || ha.BirthDay.Month == 02);

                foreach (var item2 in check) {
                    Console.WriteLine(item2);
                }

                //    5) Borcu, umumi borclularin orta borcunnan cox olan borclulari cixarmaq ve evvel familyaya gore sonra ise meblegin azalmagina gore sortirovka etmek

                Console.WriteLine("5....");
                var t = debtors.Average(f => f.Debt);
                var rr = debtors.Where(ft => t < ft.Debt);
                Console.WriteLine($"Orta borc : {t} ");
                Console.WriteLine("Orta borcdan chox borcu olan borclular sort FullName Gore");
                var sorteddebtors = rr.OrderBy(d => d.FullName);
                var sorteddebtors1 = rr.OrderByDescending(d => d.Debt);
                Console.WriteLine();
                foreach (var item34 in sorteddebtors) { Console.WriteLine(item34); }
                Console.WriteLine();
                Console.WriteLine("Orta borcdan chox borcu olan borclular sort meblegin azalmasina gore");
                Console.WriteLine();
                foreach (var item341 in sorteddebtors1) { Console.WriteLine(item341); }
                Console.WriteLine();
                ...
                var resultr = 0;
                Console.WriteLine();
                var pp = debtors.Where(num => !num.Phone.Contains("8"));
                foreach (var ppp in pp)
                {
                    string[] fullNamesParts = ppp.FullName.Split(' ');
                    string surNAme = fullNamesParts[0];
                    Console.Write($"SurName: {surNAme} => ");
                    resultr += ppp.Debt;
                    Console.WriteLine($"Age : {DateTime.Now.Year - ppp.BirthDay.Year}");
                }
                Console.WriteLine($"Telefon nomresinde 8 olmayan borclularin Umumi Borcu : {resultr}\n");
                ...
                //{
                //    //for (int j = 0;j < 
                //    //debtors.Where(a => a.FullName.Count(a.FullName.Any()));
                //    //debtors.Where(a => debtors[i].FullName.Count(c => c == debtors[i]) > 1); /////////// sinif
                //}

                //    8)borclulardan en coxu hansi ilde dogulubsa hemin ili cixartmaq
                Console.WriteLine();
                Console.WriteLine("8....");
                var birthYearCounts = debtors.GroupBy(d => d.BirthDay.Year)
                                    .Select(g => new { Year = g.First().BirthDay.Year, Count = g.Count() })
                                    .OrderByDescending(g => g.Count);

                int maxCount = birthYearCounts.First().Count;
                var Years = birthYearCounts.Where(g => g.Count == maxCount).Select(g => g.Year);

                Console.Write("En Cox Borclunun Tevellud Ili : ");
                ...
                Console.WriteLine("9....");
                var fffffff = debtors.OrderByDescending(d => d.Debt).SkipLast(debtors.Count - 5);
                foreach (var item in fffffff)
                {
                    Console.WriteLine(item);
                }
                //    10)Butun borcu olanlarin borcunu cemleyib umumi borcu cixartmaq
                int result11 = 0;
                var borc = debtors.Where(borclu => borclu.Debt != 0);
                for (int i = 0; i < debtors.Count; i++)
                {
                    result11 += debtors[i].Debt;
                }
                Console.WriteLine("10....");
                Console.WriteLine();
                Console.WriteLine($"Umumi Borc : {result11}");
                Console.WriteLine();

                //    11)2ci dunya muharibesin gormush borclularin siyahisi cixartmaq
                Console.WriteLine("11....");
                var secondww = debtors.Where(ww => ww.BirthDay.Year < 1945 || (ww.BirthDay.Year == 1945 && ww.BirthDay.Month < 9) || ww.BirthDay.Year == 1945 && ww.BirthDay.Month == 9 && ww.BirthDay.Day <= 2);
                foreach (var item1 in secondww) { Console.WriteLine(item1); }
                Console.WriteLine();
            ...
            Console.WriteLine("13....");
                var b = debtors.Where(A =>(((A.BirthDay.Month) - (DateTime.Now.Month)) >= 0 ? (((A.BirthDay.Month) - (DateTime.Now.Month)) * 500) : ((A.BirthDay.Month) + (DateTime.Now.Month)) * 500) >= A.Debt);
                foreach (var item221 in b)
                {
                    Console.WriteLine(item221);
                }
                    Console.WriteLine();
                    Console.WriteLine();
                ...
                var s = debtors.Where(ss => ss.FullName.ToLower().Contains("s") && ss.FullName.Contains("m") && ss.FullName.Contains("i") && ss.FullName.Contains("l") && ss.FullName.Contains("e"));
                foreach (var item222 in s)
                {
                    Console.WriteLine(item222);
                }
            }
        }
    }
